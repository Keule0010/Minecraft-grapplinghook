...
public class Commands implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
		if (args.length > 1) {
			...
			if (subCmd.equalsIgnoreCase("setPrefix")) {
				...
				if (s.isOp() || s.hasPermission("grapplinghook.cmd.setName")
						|| s.hasPermission("grapplinghook.cmds.*")) {
					final String value = conactStrings(args, 1);
					ConfigManager.set(ConfigKeys.NAME, value);
		...
		if (args.length == 1) {
			...
			if (subCmd.equalsIgnoreCase("reload") || subCmd.equalsIgnoreCase("rl"))
				...
				if (s.isOp() || s.hasPermission("grapplinghook.cmd.resetConfig")
						|| s.hasPermission("grapplinghook.cmds.*")) {
	...
	private String conactStrings(String[] args, int offset) {
		String value = "";
	...
	private void addUses(CommandSender s, Player p, int uses) {
		...
		if (lore == null || lore.isEmpty()) {
			...
			try {
				...
				s.sendMessage("§cThe lore of a grappling hook from player: " + p.getName()
	...
	private void givePlayerGh(CommandSender s, Player p) {
		if (p == null)
			return;
		if (s.isOp() || s.hasPermission("grapplinghook.cmd.give") || s.hasPermission("grapplinghook.cmds.*"))
	...
	private void reloadConfig(CommandSender s) {
		if (s.isOp() || s.hasPermission("grapplinghook.cmd.reload") || s.hasPermission("grapplinghook.cmds.*")) {
			ConfigManager.reload();
	...
	private void sendHelp(CommandSender s, int i) {
		...
		s.sendMessage("§3-------------->" + ConfigManager.getPrefix() + "§7 Page: " + i + "/7§3<--------------");
		if (i == 0) {
			...
			s.sendMessage("§7/gh setPrefix [String] | §2Changes the prefix of the plugin.");
			s.sendMessage("§7/gh setName [String] | §2Changes the name of the GH.");
		} else if (i == 4) {
			...
			s.sendMessage(
					"§7/gh addUses [PlayerName] [Number] | §2Adds to the uses of the GH in [PlayerName] hand new uses.");
			s.sendMessage("§7/gh setPlayerUses [Number] | §2Sets the uses of the GH in your hand.");
			s.sendMessage(
					"§7/gh setPlayerUses [PlayerName] [Number] | §2Sets the uses of the GH in [PlayerName] hand.");
		} else if (i == 5) {
			...
			s.sendMessage(
					"§7/gh useFloatingBlocks | §2Enable/Disable using floating blocks in the air as a pull object.");
	...
	private final List<String> cmds = Arrays.asList(new String[] { "reload", "help", "resetConfig", "allWorlds",
			"addWorld", "removeWorld", "give", "setPullSound", "setDestroySound", "setUses", "setGravity", "setSpeed",
			"setDestroyDelay", "setPrefix", "setName", "setCooldown", "addUses", "setPlayerUses", "useAir", "crafting",
			"unlimited", "enchanted", "noFallDamage", "setDestroy", "useFloatingBlocks", "unbreakable", "allRods" });

	@Override
	public List<String> onTabComplete(CommandSender s, Command cmd, String label, String[] args) {
		List<String> re = new ArrayList<>();
		if(!s.isOp() || !s.hasPermission("grapplinghook.cmd.autoComplete") || !s.hasPermission("grapplinghook.cmds.*"))
			return re;
